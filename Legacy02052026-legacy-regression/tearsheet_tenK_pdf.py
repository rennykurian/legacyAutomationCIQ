...
async def generate_ten_kPDF_from_tearsheet():
    playwright = None
    browser = None
    page = None
    ...
    try:
        ...
        # -------------------------------
        # STEP 2: FIND PDF BUTTON
        # -------------------------------
        print("📄 Searching for PDF button (#pdfImg)...")

        pdf_button = tenk_page.locator("#pdfImg")

        if await pdf_button.count() == 0:
            print("⚠️ Not found in popup, checking frames...")

            for frame in tenk_page.frames:
                candidate = frame.locator("#pdfImg")
                if await candidate.count() > 0:
                    pdf_button = candidate
                    print(f"✅ Found in frame: {frame.url}")
                    break

        if await pdf_button.count() == 0:
            print("❌ PDF button not found anywhere.")
            return

        await pdf_button.wait_for(state="visible", timeout=15000)
...
if __name__ == "__main__":
    asyncio.run(generate_ten_kPDF_from_tearsheet())