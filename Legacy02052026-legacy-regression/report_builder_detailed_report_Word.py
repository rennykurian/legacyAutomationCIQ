...
async def generate_and_save_detailed_report_word():
    ...
    try:
        ...
        await page.wait_for_function(just use doule quotes 
        () => {
        ...
        and here also double quotes but three times)

        # Use the actual Entity Selector search box (avoids the global SearchTopBar)
        search_box = (
        "#_rptOpts__rptOptsDS__optsDs__optsTog_float_esModal__"
        "rptOpts__rptOptsDS__optsDs__optsTog_float_esModal__es_searchbox"
    )

        await page.wait_for_selector(search_box, state="visible", timeout=30000)
        ...
        async with page.context.expect_page(timeout=30000) as popup_info:
            await page.click("#RepBldrTemplateImg5")
        ...
        path = "downloads/Detailed_Report.docx"
...
async def generate_and_save_detailed_report_word():
    ...
    try:
        ...
        await dl_btn.wait_for(state="visible", timeout=360000)
        ...
        save_path = os.path.join(save_folder, "Detailed_Report.docx")
