...
async def generate_save_quick_report_fromTearsheet():
    playwright = None
    browser = None
    page = None
    ...
    try:
        ...
        report_button = page.locator(
            "img[title='Company Tearsheet One Page Report in PDF']"
        )
        
        if not await report_button.is_visible():
            print("❌ 'Quick Report' button is not visible.")
            return None
...
if __name__ == "__main__":
    asyncio.run(generate_save_quick_report_fromTearsheet())